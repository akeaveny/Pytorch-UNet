...
def train_net(net,
    ...
    if config.TRAIN_ON_SUBSET:
        ...
        val_idx = np.random.choice(np.delete(total_idx, train_idx), size=int(config.NUM_VAL), replace=False)
    ...
    writer = SummaryWriter(comment=f'_Pytorch-UNET_{config.EXPERIMENT}')
    global_step = 0

    logging.info(f'''Starting training:
        Model:           {config.SEG_MODEL}
    ...
    if config.SEG_MODEL == 'deeplab':
        optimizer = optim.RMSprop(net.optim_parameters(lr=lr), lr=lr, weight_decay=config.WEIGHT_DECAY, momentum=config.MOMENTUM)
    else:
        optimizer = optim.RMSprop(net.parameters(), lr=lr, weight_decay=config.WEIGHT_DECAY, momentum=config.MOMENTUM)
    ...
    if save_cp:
        torch.save(net.state_dict(), config.MODEL_SAVE_PATH + "Best_Seg_{:.5}_Fwb_{}_Epochs.pth".format(best_Fwb, config.EPOCHS))
