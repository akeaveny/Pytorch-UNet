...
class DeepLabv3_plus_multi(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True):
        ...
        super(DeepLabv3_plus_multi, self).__init__()
        ...
        # TODO: this is the same as last_conv1()
...
if __name__ == "__main__":
    model = DeepLabv3_plus_multi(nInputChannels=3, n_classes=7, os=16, pretrained=True, _print=True)
    model.eval()
    image = torch.randn(1, 3, 512, 512)
    print("Image: ", image.size())
    with torch.no_grad():
        output1, output2 = model.forward(image)
