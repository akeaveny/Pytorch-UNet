...
def eval_net(net, upsample, loader, writer, best_Fwb, global_step, device, path=config.VAL_PRED_DIR_PATH):
    ...
    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for batch in loader:
            ...
            for i in range(config.BATCH_SIZE):
                gt_mask_addr = path + str(img_idx) + config.GT_SAVE_MASK_EXT
                gt_test = np.squeeze(gt[i, :, :, :])
                Image.fromarray(gt_test).save(gt_mask_addr)

                pred_mask_addr = path + str(img_idx) + config.PRED_MASK_EXT
    ...
    Fwb = weightedFb(path=path, aff_start=config.AFFORDANCE_START, aff_end=config.AFFORDANCE_END,
