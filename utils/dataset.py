...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.take_center_crop:
            img = self.crop(img,self.crop_h, self.crop_w, is_img=True)
            depth = self.crop(depth,self.crop_h, self.crop_w, is_img=True)
            mask = self.crop(mask, self.crop_h, self.crop_w, is_img=False)

        if self.apply_imgaug:
            img, depth, mask = np.array(img), np.array(depth), np.array(mask)
            H, W, C = img.shape[0], img.shape[1], img.shape[2]
            H_Depth, W_Depth, C_Depth = depth.shape[0], depth.shape[1], depth.shape[2]

            concat_img = np.zeros(shape=(H, W, C+1))
            concat_img[:, :, :C] = img
            concat_img[:, :, -1] = depth[:, :, 0]
...
if __name__ == '__main__':
    ...
    DATA_DIR = '/data/Akeaveny/Datasets/arl_dataset/ARLGAN/syn/tools/'
    ...
    dst = BasicDataset(imgs_dir=RGB_DATA_DIR, masks_dir=LABELS_DATA_DIR, depth_dir=DEPTH_DATA_DIR,
                       ...
                       mask_suffix='_aff_mask', depth_suffix='_depth')
