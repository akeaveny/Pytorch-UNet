...
if __name__ == "__main__":
    ...
    logging.info(f'Loaded model {config.SEG_MODEL}!')
    ...
    net.load_state_dict(torch.load(config.PREDICT_SAVED_WEIGHTS, map_location=device))
    net.eval()

    logging.info(f'Loaded saved weights {config.PREDICT_SAVED_WEIGHTS}!')
    ...
    for idx, image_num in enumerate(test_files):
        ...
        # logging.info(f'\tpred_gt_mask_addr: {pred_gt_mask_addr}')
    ...
    # get Fwb score
    Fwb = weightedFb(path=config.TEST_PRED_DIR_PATH, aff_start=config.AFFORDANCE_START, aff_end=config.AFFORDANCE_END,
